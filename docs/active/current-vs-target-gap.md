...
V40.1A-V40.7 status: V40.1A Direct Local Runner smoke passed scoped; historical V40.1A WebUI smoke is blocked and superseded for active routing; V40.2 no-WebUI workflow contract passed scoped; V40.3 generated local candidates but failed visual target-experience review; V40.3R direct img2img failed visual review; V40.3R identity-conditioned runner compatibility was repaired in V40.3R2, but both the first generated set and the stylized retry failed visual target-experience review; same-sample manual/import assets are fallback only with source/license/visual acceptance evidence; V40.4-V40.7 remain No-Go until at least two visually reviewed same-sample candidates are safe to normalize.
V40.3R3 next work: candidate-source decision inside the existing V40 stage. It must choose accepted manual/import first, a materially different Direct Local Runner route, or failed/blocked with V39 fallback. It must not create V41, reintroduce WebUI/ComfyUI, or treat V40.3R2 failed outputs as readiness.
