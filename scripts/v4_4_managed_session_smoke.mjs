...
async function main() {
  ...
  await settle(11000);
  recordCase("rate limit window settled", "passed", "waited before managed session smoke");

  const fakeCodex = createFakeCodexExecJsonl();

  const simple = launchManagedSession("V44 Simple", fakeCodex, "simple");
  if (simple.json.instanceId) createdInstances.push(simple.json.instanceId);
  assertCase("simple managed session exits 0", simple.ok, `status=${simple.status} reasonCode=${simple.reasonCode || "none"}`);
  assertCase("simple managed session mode recorded", simple.json.raw?.monitor?.sessionMode === "exec", "sessionMode=exec");
  assertCase("simple managed session binding id recorded", /^bind_managed_/.test(simple.json.raw?.monitor?.bindingId || ""), "bindingId=bind_managed_*");
  assertCase("simple observed turn.started", hasEvent(simple, "turn.started"), "eventType=turn.started");
  assertCase("simple mapped thinking", hasState(simple, "thinking"), "state=thinking");
  assertCase("simple final state success", stateOf(listInstances(), simple.json.instanceId) === "success", "state=success");

  await settle(1200);
  const success = launchManagedSession("V44 Tool Success", fakeCodex, "tool-success");
  if (success.json.instanceId) createdInstances.push(success.json.instanceId);
  assertCase("tool success managed session exits 0", success.ok, `status=${success.status} reasonCode=${success.reasonCode || "none"}`);
  ...
  await settle(1200);
  const failure = launchManagedSession("V44 Tool Failure", fakeCodex, "tool-failure");
  if (failure.json.instanceId) createdInstances.push(failure.json.instanceId);
  assertCase("tool failure managed session exits nonzero", !failure.ok && failure.reasonCode === "codex_process_failed", `status=${failure.status} reasonCode=${failure.reasonCode || "none"}`);
  ...
  assertCase("tool failure summary marks failure", failure.json.raw?.monitor?.observedFailureSignal === true, "observedFailureSignal=true");
  assertCase("tool failure final state error", stateOf(listInstances(), failure.json.instanceId) === "error", "state=error");
...
function launchManagedSession(name, fakeCodex, scenario) {
  return runPetctl([
    "codex",
    "session",
    "start",
...
function createFakeCodexExecJsonl() {
  const dir = mkdtempSync(join(tmpdir(), "agent-pet-v44-"));
